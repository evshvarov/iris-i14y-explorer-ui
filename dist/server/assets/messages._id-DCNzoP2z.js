...
import { t as Route } from "./messages._id-CMjMNmQI.js";
...
import { t as MarkdownContent } from "./markdown-content-BKz9O9gj.js";
import * as React from "react";
import { useEffect, useMemo, useState } from "react";
import { Link, useNavigate } from "@tanstack/react-router";
import { Fragment, jsx, jsxs } from "react/jsx-runtime";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { AlertCircle, ArrowLeft, Check, ChevronLeft, ChevronRight, Copy, EyeOff, Info, Lock, Send } from "lucide-react";
...
function MessageDetailPage() {
	...
	const neighborList = useQuery({
		queryKey: ["messages", "neighbor-list"],
		queryFn: () => apiFetch(`/messages?limit=500`),
		retry: 0,
		staleTime: 3e4
	});
	const { prevId, nextId } = useMemo(() => {
		const ids = (neighborList.data?.items ?? []).map((it) => String(it.messageId));
		...
		return {
			prevId: idx < ids.length - 1 ? ids[idx + 1] : void 0,
			nextId: idx > 0 ? ids[idx - 1] : void 0
		};
	...
	useEffect(() => {
		const onKey = (e) => {
			...
			if (e.key === "ArrowLeft" && prevId) navigate({
				to: "/messages/$id",
				params: { id: prevId }
			});
			if (e.key === "ArrowRight" && nextId) navigate({
				to: "/messages/$id",
				params: { id: nextId }
			});
		};
	...
	const componentsQuery = useQuery({
		...
		queryFn: () => apiFetch(`/productions/${encodeURIComponent(productionName)}/components`),
		enabled: !!productionName,
		staleTime: 300 * 1e3,
		retry: 0
	});
	const componentNames = useMemo(() => {
		return (componentsQuery.data?.items ?? componentsQuery.data?.components ?? []).map((c) => c.name).filter((n) => !!n);
	...
	return /* @__PURE__ */ jsxs(Fragment, { children: [/* @__PURE__ */ jsx(PageHeader, {
		...
		title: `#${id}`,
		status: m ? {
			label: m.statusLabel ?? m.statusName ?? m.status ?? "unknown",
		...
		actions: /* @__PURE__ */ jsxs("div", {
			...
			children: [
				/* @__PURE__ */ jsxs("div", {
					className: "flex items-center rounded-md ring-1 ring-black/5 bg-card overflow-hidden",
					children: [
						prevId ? /* @__PURE__ */ jsxs(Link, {
							to: "/messages/$id",
							params: { id: prevId },
							className: "flex items-center gap-1 text-xs px-2.5 py-1.5 hover:bg-muted",
							title: `Previous message #${prevId} (←)`,
							children: [/* @__PURE__ */ jsx(ChevronLeft, { className: "size-3.5" }), /* @__PURE__ */ jsxs("span", {
								className: "font-mono",
								children: ["#", prevId]
							})]
						}) : /* @__PURE__ */ jsx("span", {
							className: "flex items-center gap-1 text-xs px-2.5 py-1.5 text-muted-foreground/50 cursor-not-allowed",
							children: /* @__PURE__ */ jsx(ChevronLeft, { className: "size-3.5" })
						}),
						/* @__PURE__ */ jsx("span", { className: "w-px h-5 bg-black/5" }),
						nextId ? /* @__PURE__ */ jsxs(Link, {
							to: "/messages/$id",
							params: { id: nextId },
							className: "flex items-center gap-1 text-xs px-2.5 py-1.5 hover:bg-muted",
				...
				productionName ? /* @__PURE__ */ jsxs("button", {
				...
				productionName ? /* @__PURE__ */ jsxs(Link, {
					to: "/productions/$name",
		...
		children: [
			detail.isLoading ? /* @__PURE__ */ jsx(Skeleton, { className: "h-24 rounded-lg" }) : detail.error ? /* @__PURE__ */ jsx(ErrorPanel, {
				...
				children: [
					...
					/* @__PURE__ */ jsx(MetaLink, {
						label: "Source",
						name: m.sourceConfigName,
						productionName
					}),
					/* @__PURE__ */ jsx(MetaLink, {
						label: "Target",
						name: m.targetConfigName,
						productionName
					}),
					/* @__PURE__ */ jsx(Meta, {
						label: "Invocation",
			...
			explain.data?.explanation?.text || explain.data?.summary ? /* @__PURE__ */ jsxs("section", {
				...
				children: [
					...
					explain.data.summary ? /* @__PURE__ */ jsx(MarkdownContent, {
						className: "mb-2",
						linkComponents: componentNames,
						productionName,
						children: explain.data.summary
					}) : null,
					explain.data.explanation?.text ? /* @__PURE__ */ jsx(MarkdownContent, {
						linkComponents: componentNames,
						productionName,
				...
				/* @__PURE__ */ jsxs("section", { children: [/* @__PURE__ */ jsxs("div", {
					...
					children: steps.map((s, i) => /* @__PURE__ */ jsxs("li", {
						...
						children: [
							/* @__PURE__ */ jsxs("div", {
								...
								children: [
									...
									s.isError ? /* @__PURE__ */ jsxs("span", {
										...
										children: [/* @__PURE__ */ jsx(AlertCircle, { className: "size-3" }), s.statusLabel ?? s.statusName ?? s.status]
									}) : /* @__PURE__ */ jsx("span", {
										className: "text-[10px] font-mono uppercase text-muted-foreground bg-muted rounded px-1.5 py-0.5",
										children: s.statusLabel ?? s.statusName ?? s.status ?? "ok"
									}),
									/* @__PURE__ */ jsx(EvidencePopover, {
							...
							s.explanation ? /* @__PURE__ */ jsx("div", {
								className: "mt-3",
...
function MetaLink({ label, name, productionName }) {
	return /* @__PURE__ */ jsxs("div", {
		className: "p-3 bg-card ring-1 ring-black/5 rounded-lg",
		children: [/* @__PURE__ */ jsx("div", {
			className: "text-[9px] font-semibold uppercase tracking-widest text-muted-foreground mb-1",
			...
			title: name ?? "—",
			children: name ? productionName ? /* @__PURE__ */ jsx(Link, {
				to: "/productions/$name/components/$componentName",
				params: {
					name: productionName,
					componentName: name
...
function CopyButton({ getText, label = "Copy" }) {
	...
	return /* @__PURE__ */ jsxs("button", {
		...
		title: label,
		children: [copied ? /* @__PURE__ */ jsx(Check, { className: "size-3" }) : /* @__PURE__ */ jsx(Copy, { className: "size-3" }), copied ? "copied" : label]
	});
}
			...
			/* @__PURE__ */ jsxs("div", {
				...
				children: [/* @__PURE__ */ jsx("h2", {
					...
					children: [
						...
						/* @__PURE__ */ jsxs("span", {
							className: "text-[10px] font-mono uppercase text-muted-foreground",
							title: "Payload metadata flag",
							children: ["metadata: ", supported ? metadataEnabled ? "enabled" : "disabled" : "unsupported"]
						}),
						/* @__PURE__ */ jsxs("span", {
							className: "text-[10px] font-mono uppercase text-muted-foreground",
							title: "Payload inspection flag",
							children: ["inspection: ", supported ? inspectionEnabled ? "enabled" : "disabled" : "unsupported"]
						}),
			...
			/* @__PURE__ */ jsxs("div", {
				...
				children: [/* @__PURE__ */ jsx("h2", {
					...
					children: [
						...
						fields.length > 0 ? /* @__PURE__ */ jsxs(Fragment, { children: [/* @__PURE__ */ jsx(CopyButton, {
							label: "Copy TSV",
							getText: () => fields.map((f) => `${f.name ?? ""}\t${f.type ?? ""}\t${f.value ?? ""}`).join("\n")
						}), /* @__PURE__ */ jsx(CopyButton, {
							label: "Copy JSON",
							getText: () => JSON.stringify(data, null, 2)
						})] }) : null
